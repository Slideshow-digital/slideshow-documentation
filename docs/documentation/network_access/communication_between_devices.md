...
If the devices are on different networks, we also enabled a special feature called **Tunnel web interface**. It creates a network tunnel between the two devices, using which a web browser on your computer can access the [web interface](web_interface.md) of Slideshow installed on the remote device. This tunnel uses UDP protocol (as required for the Hole punching method mentioned above), and its transfer speed is usually much lower than the regular network speed. As Slideshow's web interface is quite lightweight, the tunnel is still fast enough to provide access to all management features.
