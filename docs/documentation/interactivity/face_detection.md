...
Face detection can’t be used at the same time as [video input](../playback/special_content/video_input.md) from the same camera, as two different processes can’t use the same camera at the same time.
