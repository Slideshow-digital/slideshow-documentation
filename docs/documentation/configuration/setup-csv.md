...
- In ZIP archive, which can be uploaded through web interface, FTP, USB Flash drive or downloaded from internet via [File synchronization](../network-access/file-synchronization/index.md).
- In [Google Drive](../network-access/file-synchronization/file-synchronization-google-drive.md) or [Dropbox](../network-access/file-synchronization/file-synchronization-dropbox.md) folder, which is synchronized via File synchronization
- In "slideshow" folder on the [USB Flash drive](file-storage.md)
...
- The line starts with colon `:` - special command:
    ...
    - `:CONFIG;file name;delete` - restore [XML configuration backup](configuration-backup-restore.md) from the file. `External config import` has to be enabled in [Device settings](settings.md) in order for this functionality to work (this setting acts a security check). The part `delete` means that the previous configuration will be replaced. Skip this part if you want to just add the new configuration and keep the previous one at the same time.
    - `:UPDATE;APK file name;version` - [remotely update](remote-update.md) Slideshow application. The version part should be set to the version of Slideshow app which is in the APK file, it is used as a check if the app is already updated or not (e.g. "3.13.0"). "External config import" has to be enabled in Device settings in order for this functionality to work (this setting acts as a security check). Works only on [rooted devices](../hardware/root-on-android.md).
    ...
    - `:REBOOT` - reboot the Android device after processing of the files is finished. "External config import" has to be enabled in Device settings in order for this functionality to work (this setting acts as a security check). Works only on [rooted devices](../hardware/root-on-android.md).
- Other lines - definition from when until when should the file be displayed/played (provided that it will be in a playlist). The order is file name, date from, date to. One of the dates may be an empty string. If date to is in the future, the file will be [automatically deleted](playback-configuration/scheduled-file-deletion.md) at that date.
...
```
02.01.20 18:43:26 DEBUG 
sk.mimac.slideshow.csv.FileDataParser - Detected file data file 'setup.csv'
```
