...
The boot animation is usually prepared by the manufacturer of the device and contains their logo. If you are using an Android device for digital signage, it might be a good idea to replace the original boot animation with something more professional-looking. This is possible only on [rooted devices](../hardware/root-on-android.md), otherwise the original boot animation can't be replaced.
