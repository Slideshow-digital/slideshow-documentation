...
You can access all files uploaded to Slideshow using a WebDAV client (for example, [WinSCP](https://winscp.net/)). WebDav interface is running on the same port as [HTTP and HTTPS interface](web-interface.md), on path `/webdav`. Use the following connection settings:
