...
- **Scaling videos:** the videos within the zone are scaled with the same options as images ([more information](../on-screen_formatting/scaling_images.md)).
- **Rotating videos:** together with rotating the screen layout, it also rotates the video in the correct way ([more information](../../configuration/screen_rotation.md)).
- **Better support for video streams:** ExoPlayer has better support for streaming through network protocols, e.g. HTTP / RTSP / UDP / RTMP ([more information](../special_content/video_stream.md)).
