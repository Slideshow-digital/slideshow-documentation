...
!!! warning "Only local access"
    Direct file upload is accessible only within the same local network. It is not accessible remotely through internet.

## FTP interface

You can access all files uploaded to Slideshow using a FTP client (for example [WinSCP](https://winscp.net/) or [Total Commander](https://www.ghisler.com/)	). Use the following connection settings:
...
FTP protocol is not encrypted (Slideshow doesn’t support SFTP or FTP over SSL). If you are connecting to the device via public or not-secured Wi-Fi (Wi-Fi without password), you connection might be easily intercepted.
